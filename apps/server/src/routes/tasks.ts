...
function parseColumn(raw: unknown): TaskColumn | null {
  if (typeof raw !== "string") return null;
  return COLUMNS.includes(raw as TaskColumn) ? (raw as TaskColumn) : null;
}
...
tasksRouter.post("/", requireManagerOrAdmin, async (req: Request, res: Response): Promise<void> => {
  const body = req.body as { title?: string; column?: string };
...
tasksRouter.patch("/:id", requireManagerOrAdmin, async (req: Request, res: Response): Promise<void> => {
...
tasksRouter.delete("/:id", requireManagerOrAdmin, async (req: Request, res: Response): Promise<void> => {
  const id = req.params.id;
