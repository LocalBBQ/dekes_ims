...
inventoryRouter.post("/import", upload.single("file"), async (req, res) => {
    try {
        const file = req.file;
        if (!file?.buffer) {
        ...
        for (const row of rows) {
            ...
            if (lastOrderRaw != null) {
                if (typeof lastOrderRaw === "number" && lastOrderRaw > 0) {
                    const parsed = XLSX.SSF.parse_date_code(lastOrderRaw);
                    if (parsed)
                        lastOrderAt = new Date(parsed.y, parsed.m - 1, parsed.d);
                    else
                        lastOrderAt = new Date((lastOrderRaw - 25569) * 86400 * 1000);
                }
...
inventoryRouter.post("/", async (req, res) => {
    try {
        ...
        for (const loc of locations) {
            ...
            const qty = typeof raw === "number" ? raw : (raw && typeof raw === "object" && typeof raw.quantity === "number" ? raw.quantity : 0);
            const inUse = raw && typeof raw === "object" && typeof raw.quantityInUse === "number" ? raw.quantityInUse : 0;
            await prisma.itemLocationQuantity.create({
...
inventoryRouter.patch("/:id/quantity", async (req, res) => {
    try {
        const id = req.params.id;
        const body = req.body;
...
inventoryRouter.patch("/:id/transfer", async (req, res) => {
    try {
        const id = req.params.id;
        const body = req.body;
...
inventoryRouter.patch("/:id", async (req, res) => {
    try {
        ...
        if (body.reorderLink !== undefined)
            updateData.reorderLink = body.reorderLink?.trim() || null;
