...
imagesRouter.post("/:itemId/images", upload.single("file"), async (req, res) => {
    const itemId = req.params.itemId;
    const fieldDefinitionId = req.body?.fieldDefinitionId || null;
    const file = req.file;
    if (!file) {
...
imagesRouter.get("/:itemId/images/:imageId", async (req, res) => {
...
imagesRouter.delete("/:itemId/images/:imageId", async (req, res) => {
    const { itemId, imageId } = req.params;
    const image = await prisma.itemImage.findFirst({
        where: { id: imageId, itemId },
    });
